...
(\**) L'evento RECAG012 rappresenta l'accettazione del 23L. Lo `statusDateTime` dovrà essere valorizzato con la seguente logica: data di ritiro in caso di consegna in giacenza antecedente i 10 giorni dall'invio della CAD, oppure data di invio della CAD + 10 giorni. La macchina a stati non risente della produzione di questo evento, il flusso di rendicontazione continua a partire dal precedente evento ricevuto.
...
stateDiagram-v2
  ...
  Inesito --> GestioneGiacenza: RECAG011A
  ...
  PresaInCarico --> NonRendicontabile:RECAG013 (***)
  NonRendicontabile --> [*]
  
  %% Causa forza maggiore
  CausaForzaMaggiore --> PresaInCarico
  PresaInCarico --> CausaForzaMaggiore:RECAG015 (****)
  ...
  Preesito3i:Preesito
  Demat3i:Demat
  GestioneGiacenza: Gestione in giacenza
...
stateDiagram-v2
  ...
  state GestioneGiacenza {
    ...
    InGiacenza --> MancataConsegnatoPressoGiacenza: RECAG007A
    InGiacenza --> ConsegnatoPressoGiacenzaPersonaAbilitata: RECAG006A
    InGiacenza --> ConsegnatoPressoGiacenza: RECAG005A
